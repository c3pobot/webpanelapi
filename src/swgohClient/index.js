...
let retryCount = 6, apiStatus = false
...
const checkStatus = async()=>{
  try{
    ...
    let res = await requestWithRetry(path.join(CLIENT_URL, 'metadata'), opts )
    if(res?.body?.latestLocalizationBundleVersion){
      apiStatus = true
      log.info('swgoh api connection succesful...')
      return
    }
